    ...
    <div className='pt-28 px-8 py-12 min-h-[85vh]'>
      ...
      {favorites.length > 0 ? (
        <>
          <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6'>
            {favorites.map((album, index) => (
              <div key={`${album.id || album.name}-${index}`}>
                <AlbumCard
                  album={album}
                  favorites={favorites}
        ...
        <div className='flex flex-col items-center justify-center py-16 px-6 text-center bg-[#3D1308] rounded-3xl border border-[#7B0D1E]/60 max-w-md mx-auto shadow-lg'>
          <div className='w-16 h-16 rounded-full bg-[#211103] flex items-center justify-center text-3xl mb-4 border border-[#7B0D1E] shadow-inner'>🎵</div>
